...
class CustomTabBar: UITabBar {
    ...
    private var centerButtonImg = configure(UIImageView()) {
        $0.image = UIImage(name: .tabBarAddPhoto)
    ...
    private func bind() {
        inTabItemTapped.compactMap { YMTabBarItemType(rawValue: $0) }
    ...
    override func point(inside point: CGPoint, with _: UIEvent?) -> Bool {
        if centerButtonImg.frame.contains(point) {
            Observable.just(point.x).map { $0 != self.latestTappedPointx }
                .filter { $0 }
                .withLatestFrom(Observable.just(YMTabBarItemType.addPhoto.rawValue))
